...
With **Sage Accounting** connected (Settings → Integrations), every payroll run you **mark as paid** is posted to Sage as a journal: one line per employee for their gross pay against your wages account, and one credit for the total to a wages-payable or accruals account. **Contractors are different, and Sage shows them properly:** each subcontractor becomes a *supplier* in Sage, every finalized statement posts a *bill* against them (one per person, hours in the description), and marking the statement paid records the supplier payment from your bank account. Open the supplier in Sage and you see exactly who you're paying, what, and what's still owed. The accounts all of this posts to are listed on the Sage card under Settings → Integrations, picked by name from your chart the first time and changeable there. It is deliberately gross, not net: Sollos doesn't know deductions or what actually left the bank — your payroll product does — so the bookkeeper clears that liability against the bank feed and the remittances. A run that didn't post is retried every few hours, and the reason (usually a missing account in Sage's chart) shows on the Integrations card in Sage's words.

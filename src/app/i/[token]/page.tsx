...
import { formatCurrencyCents, formatDate, FALLBACK_TZ } from "@/lib/format";
...
export default async function PublicInvoicePage({
  ...
  const { data: taxData } = (await admin
    .from("invoices")
    .select(
      "tax_rate_bps, tax_amount_cents, tax_label, public_token_expires_at",
    )
    .eq("id", invoice.id)
  ...
  const tz = orgId ? await getOrgTimezone(orgId) : FALLBACK_TZ;
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-2xl">
        ...
        <div className="sollos-card overflow-hidden shadow-lg sm:p-0">
            ...
            {(orgContact.contact_email || orgContact.contact_phone) && (
              <div className="mt-6 rounded-lg border border-border bg-muted/20 p-5">
                ...
                <p className="mt-1.5 text-sm text-muted-foreground">
                  Get in touch with {invoice.organization?.name ?? "the sender"}
                  :
                </p>
          ...
          {/* close p-6 sm:p-8 inner wrapper */}
        </div>
        {/* close sollos-card */}

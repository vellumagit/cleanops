...
export default async function SubcontractorPayablesPage() {
  ...
  return (
    <PageShell
      ...
      description="What you owe each contractor for completed jobs. Paid per job, never inside a payroll run."
      actions={
        <Link
          href="/app/payroll"
                    ...
                    >
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-2">
                          <Link
                            href={`/app/payroll/contractors/${encodePayeeParam(r.payee)}`}
                        ...
                        {/* Where they came from is a LABEL, not a section.
                            Both are paid here, identically; the only real
                            difference is how the amount is derived — clocked
                            hours for your own roster, the agreed price for a
                            claimed offer from the bench. That's a column. */}
                        <p className="mt-0.5 text-[10px] uppercase tracking-wide text-muted-foreground">
                          {r.isRoster
                            ? "Your roster · paid on clocked hours"
                            : "On-call · paid per claimed shift"}
                        </p>
                      </td>
